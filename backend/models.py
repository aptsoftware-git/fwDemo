...
class RemoteWorkshop(Base):
    """
    Represents data from Remote Workshop (Eng, EOA Spares, MUA sheets).
    Unit + Category is the composite primary key and links to A Vehicle data.
    """
    __tablename__ = "remote_workshop"
    
    id = Column(Integer, primary_key=True, index=True)
    dataset_id = Column(Integer, ForeignKey("datasets.id", ondelete="CASCADE"), nullable=False)
    sheet_name = Column(String(50), nullable=False, index=True)  # "Eng", "EOA Spares", or "MUA"
    unit = Column(String(100), nullable=False, index=True)
    category = Column(String(255), nullable=False, index=True)
    row_data = Column(JSON, nullable=False)  # All column data from respective sheet
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    ...
    def __repr__(self):
        return f"<RemoteWorkshop(id={self.id}, sheet='{self.sheet_name}', unit='{self.unit}', category='{self.category}')>"
